...
public class ShiroConfig {
	...
	public JdbcRealm jdbcRealm() {
		...
		// 根据用户名查询角色
		realm.setUserRolesQuery("SELECT rname FROM user_role ur INNER JOIN USER u ON ur.`uid` = u.`uid` INNER JOIN role r ON ur.rid = r.rid AND uname = ?");
		// 根据角色名查询权限
		realm.setPermissionsQuery("SELECT pmsname FROM role_permission rp INNER JOIN role r ON rp.`rid` = r.`rid` INNER JOIN permission p ON rp.`pmsid` = p.`pmsid` AND rname = ?");
